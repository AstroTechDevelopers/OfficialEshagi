...
document.getElementById('loginForm').addEventListener('submit', submitFunction);
   ...
   // var showmessage = firebase.database().ref('newsite_merchants').orderByKey();
var email1=getID('email');
var password1=getID('password');
    function getID(id){
...
var password1=getID('password');
    document.getElementById("progress").style.visibility = "visible";
      const dbRefObject1 = firebase.database().ref().child('partner_users');
  const dbrefList1=dbRefObject1;
...
var role = snap.child('role').val();
    ...
    var partner_uid  = snap.child('partner_uid').val();
    
    if(email==email1){
        if(password==password1){
            localStorage.setItem("cname",fullname);
            localStorage.setItem("role",role);
            localStorage.setItem("partner_uid",partner_uid);
            localStorage.setItem("uid",uid);
            
            localStorage.setItem("cemail",email);
            getUserDetails(partner_uid);
        ...
        else
    {
      ...
      //  elem.insertAdjacentHTML("<tr><td>"+ referenceNumber +"</td>"+"<td>" + type + "</td>"+"<td>" + accountUsed + "</td>"+"<td>" + dd + "</td>"+"<td>" + amount + "</td>"+"<td>" + amount + "</td>"+"<td>" + balance + "</td>"+"</tr>");
    });
    
}

 function getUserDetails(licenceNumber){
     
     var licence_number = licenceNumber;
     console.log(licence_number);
var ref2 = firebase.database().ref("newsite_merchants/"+licence_number);
ref2.once("value")
  .then(function(snapshot) {
    ...
    //  document.getElementById('loginForm').reset();
       window.location='partners/index.php';
    }
  });
}
