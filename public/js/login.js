var firebaseConfig = {
  apiKey: "AIzaSyD87WSXd-b9HnYbW4_xVBdrRB8kBZBh0jk",
  authDomain: "eshagi-3d1c3.firebaseapp.com",
  databaseURL: "https://eshagi-3d1c3.firebaseio.com",
  projectId: "eshagi-3d1c3",
  storageBucket: "eshagi-3d1c3.appspot.com",
  messagingSenderId: "1017983101729",
  appId: "1:1017983101729:web:a104463433112b86f42c60",
  measurementId: "G-P6DL895MWC"
};
firebase.initializeApp(firebaseConfig);
//initialise the alerts
document.querySelector('.positive').style.display = 'none'; 
document.querySelector('.amber').style.display = 'none'; 
document.querySelector('.negative').style.display = 'none'; 

document.getElementById('loginForm').addEventListener('submit', submitFunction);
    
    
    var showmessage = firebase.database().ref('newsite_reg').orderByKey();
...
function submitFunction(event){
   ...
   document.getElementById("loginToStart").innerHTML="Please wait!";
   document.getElementById("loginToStart2").innerHTML="We're just verifying your details";
    event.preventDefault();
document.getElementById("myid").style.visibility = "hidden";
document.getElementById("mypassword").style.visibility = "hidden";
...
document.getElementById("loginbtn").style.visibility = "hidden";
  setTimeout(function(){
   ...
   document.getElementById("loginToStart").innerHTML="Login To Start";
   document.getElementById("loginToStart2").innerHTML="The ID or Password your entered is incorrect";
    event.preventDefault();
document.getElementById("myid").style.visibility = "visible";
document.getElementById("mypassword").style.visibility = "visible";
...
document.getElementById("loginbtn").style.visibility = "visible";
    ...
    //get values
    var ID_number = getID('natid');
    var password = getID('password');
    document.querySelector('.spinner-grow').style.display ='block';
    //save message to firebase
    showmessage.once('value', function(snapshot){  
    if(snapshot.exists()){ 
       

        snapshot.forEach(function(data){ // loop
            var my_id = data.val().id;
            var my_password = data.val().confirm_password;
            if (my_password == password && my_id == ID_number) { 
        ...
        console.log("OLA1");
        document.querySelector('.amber').style.display = 'block'; 
        document.querySelector('.spinner-grow').style.display ='none'
        setTimeout(function(){
        document.querySelector('.amber').style.display = 'none';
        //  window.location='addressdetails.html';
        },3000);
     }
 ...
 function getUserDetails(natid){
     
     var ID_number = document.getElementById('natid').value;
     console.log(ID_number);
var ref2 = firebase.database().ref("newsite_reg/"+ID_number);
ref2.once("value")
  .then(function(snapshot) {
    if (snapshot.exists) {
        console.log(snapshot.child('firstname').val());
      var firstname  = snapshot.child('firstname').val();
      var surname  = snapshot.child('surname').val();
      var natId  = snapshot.child('id').val();
      var passPhotoPath  = snapshot.child('passPhotoPath').val();
      var phoneNumber = snapshot.child('officialNumber').val();
      localStorage.setItem("firstname", firstname);
      localStorage.setItem("surname", surname);
      localStorage.setItem("natId", natId);
      localStorage.setItem("passPhotoPath", passPhotoPath);
      localStorage.setItem("phoneNumber",phoneNumber);
      localStorage.setItem("netSalary",snapshot.child('monthly_salary').val());
      localStorage.setItem("accountNumber",snapshot.child('accountNumber').val());
      localStorage.setItem("branchCode",snapshot.child('branchCode').val());
      localStorage.setItem("branchName",snapshot.child('branchName').val());
      localStorage.setItem("bankName",snapshot.child('bankName').val());
      localStorage.setItem("passportPhoto",snapshot.child('passportPhoto').val())
    //  document.getElementById('loginForm').reset();
       window.location='Clients/index.php?idr='+natId;
